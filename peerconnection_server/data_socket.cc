...
{
	return send(socket_, data.data(), static_cast<int>(data.length()), 0) != SOCKET_ERROR;
}

bool DataSocket::Send(const std::string& status, bool connection_close, const std::string& content_type, const std::string& extra_headers, const std::string& data) const
{
	assert(valid());
	assert(!status.empty());
...
bool DataSocket::ParseMethodAndPath(const char* begin, size_t len)
{
	...
	for (size_t i = 0; i < ARRAYSIZE(supported_methods); ++i)
	{
		if (len > supported_methods[i].method_name_len &&
			...
			strncmp(begin, supported_methods[i].method_name, supported_methods[i].method_name_len) == 0)
		{
...
bool DataSocket::ParseContentLengthAndType(const char* headers, size_t length)
{
	...
	while (headers && headers < end)
	{
		if (!isspace(headers[0]))
		{
			...
			if ((headers + ARRAYSIZE(kContentLength)) < end && strncmp(headers, kContentLength, ARRAYSIZE(kContentLength) - 1) == 0)
			{
			...
			else if ((headers + ARRAYSIZE(kContentType)) < end && strncmp(headers, kContentType, ARRAYSIZE(kContentType) - 1) == 0)
			{
...
DataSocket* ListeningSocket::Accept() const
{
	...
	NativeSocket client = accept(socket_, reinterpret_cast<sockaddr*>(&addr), &size);
