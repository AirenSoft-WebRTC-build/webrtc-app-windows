...
namespace {
	...
	{
	public:
		static rtc::scoped_refptr<CapturerTrackSource> Create(int capturer_device_id = 0)
		{
			...
			capturer = absl::WrapUnique(webrtc::test::VcmCapturer::Create(kWidth, kHeight, kFps, capturer_device_id));
			if (capturer)
			{
...
Conductor::Conductor(PeerConnectionClient* client, MainWindow* main_wnd)
	: peer_id_(-1), loopback_(false), client_(client), main_wnd_(main_wnd), server_(""), capturer_device_id_(-1)
{
...
bool Conductor::CreatePeerConnection(bool dtls)
{
	...
	auto error_or_peer_connection =
		peer_connection_factory_->CreatePeerConnectionOrError(config, std::move(pc_dependencies));
...
void Conductor::OnIceCandidate(const webrtc::IceCandidateInterface* candidate)
{
	...
	auto msg = Json::writeString(factory, jmessage);
	SendMessage(msg);
}
...
void Conductor::OnMessageFromPeer(int peer_id, const std::string& message)
{
	...
	type_str = jmessage[kSessionDescriptionTypeName].asString();
	if (!type_str.empty())
	{
		...
		RTC_LOG(LS_INFO) << " Received session description :" << sdp;
	...
	else
	{
		...
		*/
		...
		if (!candidate.get())
		{
			RTC_LOG(LS_WARNING) << "Can't parse received candidate message.  SdpParseError was: " << error.description;
			return;
		}
...
//
void Conductor::SetCapturerDeviceId(int capturer_device_id)
{
	capturer_device_id_ = capturer_device_id;
}
...
void Conductor::AddTracks()
{
	...
	rtc::scoped_refptr<webrtc::AudioTrackInterface> audio_track(peer_connection_factory_->CreateAudioTrack(kAudioLabel, peer_connection_factory_->CreateAudioSource(cricket::AudioOptions()).get()));
	...
	rtc::scoped_refptr<CapturerTrackSource> video_device = CapturerTrackSource::Create(capturer_device_id_);
	if (video_device)
	{
...
void Conductor::OnSuccess(webrtc::SessionDescriptionInterface* desc)
{
	...
	RTC_LOG(LS_ERROR) << "SetLocalDescription\n" << sdp.c_str();
