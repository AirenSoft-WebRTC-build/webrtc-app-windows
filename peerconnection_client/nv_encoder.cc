...
namespace webrtc
{
	...
	static void RtpFragmentize(EncodedImage* encoded_image, const VideoFrameBuffer& frame_buffer, std::vector<uint8_t>& frame_packet)
	{
		...
		memcpy((void *)encoded_image->data(), &frame_packet[0], frame_packet.size());
		encoded_image->set_size(frame_packet.size());
	}
	...
	static void RtpFragmentize(EncodedImage* encoded_image, SFrameBSInfo* info) 
	{
		...
		for (int layer = 0; layer < info->iLayerNum; ++layer) {
			...
			for (int nal = 0; nal < layerInfo.iNalCount; ++nal, ++fragments_count) 
			{
				...
				RTC_CHECK_LE(layerInfo.pNalLengthInByte[nal],	std::numeric_limits<size_t>::max() - required_capacity);
		...
		tl0sync_limit_(0)
	{
		...
		if (codec.GetParam(cricket::kH264FmtpPacketizationMode, &packetization_mode_string) && packetization_mode_string == "1")
		{
	...
	int32_t NvEncoder::InitEncode(const VideoCodec* inst, int32_t number_of_cores, size_t max_payload_size)
	{
		ReportInit();
		...
		for (int i = 0, idx = number_of_streams - 1; i < number_of_streams; ++i, --idx)
		{
			...
			nv_encoder->SetOption(xop::VE_OPT_BFRAMES, (inst->H264().bframe_enabled == true)?1:0);
			
			/*RTC_LOG(LS_INFO) << __FUNCTION__ << " width: " << configurations_[i].width 
												<< " height: " << configurations_[i].height 
												<< " max_frame_rate: " << configurations_[i].max_frame_rate 
												<< " key_frame_interval: " << configurations_[i].key_frame_interval 
												<< " max_bps: " << configurations_[i].max_bps 
												<< " target_bps: " << configurations_[i].target_bps
												<< " bframe_enabled: " << inst->H264().bframe_enabled;*/
		...
		VideoBitrateAllocation allocation = init_allocator.GetAllocation(codec_.maxBitrate * 1000 / 2, codec_.maxFramerate);
	...
	int32_t NvEncoder::Encode(const VideoFrame& input_frame, const std::vector<VideoFrameType>* frame_types)
	{
		...
		for (size_t i = 0; i < nv_encoders_.size(); ++i)
		{
			...
			uint64_t frame_packet_pts;
			uint64_t frame_packet_dts;
			int32_t	frame_packet_cts;
			int64_t	frame_encode_finish_time_us = 0;
			
			EncodeFrame((int)i, input_frame, frame_packet, frame_packet_pts, frame_packet_dts, frame_encode_finish_time_us);
			
			// CTS = (PTS - DTS) / 90;
			frame_packet_cts = (int32_t)((int64_t)frame_packet_pts - (int64_t)frame_packet_dts) / 90;

			//RTC_LOG(LS_INFO) << __FUNCTION__ << " EncodeFrame pts=" << frame_packet_pts << " dts=" << frame_packet_dts << " cts= " << (int32_t)(frame_packet_cts) 
			//	<< " encode_finish_time=" << frame_encode_finish_time_us;
			
			if (frame_packet.size() == 0)
			{
				RTC_LOG(LS_INFO) << __FUNCTION__ << " Pending due to bframe";
				return WEBRTC_VIDEO_CODEC_OK;
			}
			else
			{
				if ((frame_packet[4] & 0x1f) == 0x07) // 이건 SPS 인데 idr을 설정, 인코더가 IDR 앞에 SPS/PPS를 붙여서 보내서 이렇게 처리했나보군, 5만 단독으로 나오는 경우는 없나?
				{
					info.eFrameType = videoFrameTypeIDR;
				}
				else if ((frame_packet[4] & 0x1f) == 0x01) // non-IDR 프레임인데 p프레임이라고 했구나, 어짜피 B가 없으니 상관은 없었겠지, 
				{
					info.eFrameType = videoFrameTypeP;
				}
				else
				{
					//RTC_LOG(LS_WARNING) << __FUNCTION__ << " EncodeFrame frame type is not supported: " << (int)(frame_packet[4] & 0x1f);
					return WEBRTC_VIDEO_CODEC_OK;
				}
			}
			...
			encoded_images_[i].SetRtpTimestamp(frame_packet_dts);
			encoded_images_[i].SetCompositionTimestamp(frame_packet_cts);

			// Split encoded image up into fragments. This also updates
			// |encoded_image_|.
			RtpFragmentize(&encoded_images_[i], *frame_buffer, frame_packet);
			...
			if (encoded_images_[i].size() > 0)
			{
				...
				//RTC_LOG(LS_INFO) << "Encoded frame " << i << " - Type: " << (int)info.eFrameType << " Length: " << encoded_images_[i].size() << " QP: " << encoded_images_[i].qp_;
	...
	void NvEncoder::ReportInit()
	{
		if (has_reported_init_)
			return;
		RTC_HISTOGRAM_ENUMERATION("WebRTC.Video.NvEncoder.Event", kH264EncoderEventInit, kH264EncoderEventMax);
	...
	void NvEncoder::ReportError()
	{
		if (has_reported_error_)
			return;
		RTC_HISTOGRAM_ENUMERATION("WebRTC.Video.NvEncoder.Event", kH264EncoderEventError, kH264EncoderEventMax);
	...
	bool NvEncoder::EncodeFrame(int index, const VideoFrame& input_frame, std::vector<uint8_t>& frame_packet, uint64_t& frame_packet_pts, uint64_t& frame_packet_dts, int64_t& encode_finish_time)
	{
		frame_packet.clear();
		...
		if (video_format_ == EVideoFormatType::videoFormatI420)
		{
			...
			{
				if (webrtc::ConvertFromI420(input_frame, webrtc::VideoType::kARGB, 0, image_buffer_.get()) < 0)
				{
					return false;
				}
			}
		...
		if (nv_encoder)
		{
			int frame_size = nv_encoder->Encode(std::vector<uint8_t>(image_buffer_.get(), image_buffer_.get() + image_size),
												input_frame.timestamp(),
												frame_packet,
												frame_packet_pts, frame_packet_dts, encode_finish_time);
			if (frame_size < 0)
			{
				return false;
			}
