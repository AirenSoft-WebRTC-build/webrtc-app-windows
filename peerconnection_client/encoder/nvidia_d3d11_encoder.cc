...
bool NvidiaD3D11Encoder::Init()
{
	...
	initialize_params.encodeConfig->frameIntervalP = (bframes_ > 0) ? 3 : 1;
	initialize_params.encodeConfig->rcParams.enableLookahead = 1; //  Lookahead
	initialize_params.encodeConfig->rcParams.disableIadapt = 1; // No Scenecut
	initialize_params.encodeConfig->rcParams.disableBadapt = 1;
	initialize_params.encodeConfig->rcParams.enableNonRefP = 0; // B-frames
...
int NvidiaD3D11Encoder::Encode(std::vector<uint8_t> in_image, uint64_t in_pts, std::vector<uint8_t>& out_frame, uint64_t& out_pts, uint64_t& out_dts, int64_t& out_finish_time)
{
	if (!nv_encoder_) {
		return -1;
	}
	...
	memset(&enc_pic_params, 0, sizeof(enc_pic_params));
	
	enc_pic_params.inputTimeStamp = in_pts;
	nv_encoder_->EncodeFrame(packets, out_pts, out_dts, out_finish_time, &enc_pic_params);
...
int NvidiaD3D11Encoder::Encode(HANDLE shared_handle, std::vector<uint8_t>& out_frame)
{
	...
	int64_t unusedPacketFinishTime;
	nv_encoder_->EncodeFrame(packets, unusedPacketPts, unusedPacketDts, unusedPacketFinishTime);
...
bool NvidiaD3D11Encoder::UpdateOption()
{
	...
	bframes_	  = GetOption(VE_OPT_BFRAMES, 0);
