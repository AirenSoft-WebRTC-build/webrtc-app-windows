...
#include "third_party/libyuv/include/libyuv/convert_argb.h"
 ...
 // Set header to get list of capture devices
...
bool MainWnd::Create()
{
	...
	wnd_ = ::CreateWindowExW(WS_EX_OVERLAPPEDWINDOW, kClassName, L"libWebRTC for bFrames", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_CLIPCHILDREN, CW_USEDEFAULT, CW_USEDEFAULT, 854, 480, NULL, NULL, GetModuleHandle(NULL), this);
...
void MainWnd::SwitchToPeerList(const Peers& peers)
{
	...
	if (auto_call_ && peers.begin() != peers.end())
	{
		...
		if (count != LB_ERR)
		{
			...
			if (selection != LB_ERR)
			{
				::PostMessage(wnd_, WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(listbox_), LBN_DBLCLK), reinterpret_cast<LPARAM>(listbox_));
			}
		}
	}
}
...
void MainWnd::OnDefaultAction()
{
	...
	if (ui_ == CONNECT_TO_SERVER)
	{
		...
		int capturer_index = SendMessage(capturer_listbox_, LB_GETCURSEL, 0, 0);
		if (capturer_index == LB_ERR)
		{
			::MessageBoxA(wnd_, "Please select a capture device", "Oops!", MB_OK);
			return;
		}

		callback_->SetCapturerDeviceId(capturer_index);
...
void MainWnd::CreateChildWindow(HWND* wnd, MainWnd::ChildWindowID id, const wchar_t* class_name, DWORD control_style, DWORD ex_style)
{
	if (::IsWindow(*wnd))
	{
		return;
	...
	*wnd = ::CreateWindowExW(ex_style, class_name, L"", style, 100, 100, 100, 100, wnd_, reinterpret_cast<HMENU>(id), GetModuleHandle(NULL), NULL);
...
void MainWnd::CreateChildWindows()
{
	...
	CreateChildWindow(&capturer_listbox_, CAPTURER_LISTBOX_ID, L"ListBox", LBS_HASSTRINGS | LBS_NOTIFY, WS_EX_CLIENTEDGE);
...
void MainWnd::LayoutConnectUI(bool show)
{
	...
	{
		{label1_, L"Server",                      10, 12, 50, 18},
		{edit1_, L"XXXyyyYYYgggXXXyyyYYYggg",     60, 10, 150, 18},
		{label2_, L":",                           210, 12, 20, 18},
		{edit2_, L"XyXyX",                        230, 10, 80, 18},
		{button_, L"Connect",                     310, 9, 200, 19},
		{capturer_listbox_, L"XCapturerListBox",   60, 35, 450, 270},
	};
	...
	if (show)
	{
		...
		{
			::MoveWindow(windows[i].wnd, static_cast<int>(windows[i].left), static_cast<int>(windows[i].top), static_cast<int>(windows[i].width), static_cast<int>(windows[i].height), TRUE);
		...
		LRESULT capturer_count = ::SendMessage(capturer_listbox_, LB_GETCOUNT, 0, 0);
		if (capturer_count == 0)
		{
			std::unique_ptr<webrtc::VideoCaptureModule::DeviceInfo> info(webrtc::VideoCaptureFactory::CreateDeviceInfo());
			if (info)
			{
				::SendMessage(capturer_listbox_, LB_RESETCONTENT, 0, 0);
				...
				for (int device_index = 0; device_index < num_devices; device_index++)
				{
					...
					info->GetDeviceName(device_index, device_name, sizeof(device_name), NULL, 0);

					char output_string[1024];
					sprintf(output_string, "[%d] %s\n", device_index, device_name);
					//OutputDebugStringA(output_string);

					AddListBoxItem(capturer_listbox_, output_string, device_index);
				}

				::SendMessage(capturer_listbox_, LB_SETCURSEL, 0, 0);
			}
		}
	}
	else
...
MainWnd::VideoRenderer::VideoRenderer(HWND wnd, int width, int height, webrtc::VideoTrackInterface* track_to_render)
