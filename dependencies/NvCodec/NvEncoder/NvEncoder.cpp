...
#define LOG(format, ...) { char output_buf[1024]; sprintf(output_buf, "[%s:%d] " format "\n", __FUNCTION__ , __LINE__, ##__VA_ARGS__); OutputDebugStringA(output_buf); }
...
void NvEncoder::EncodeFrame(std::vector<std::vector<uint8_t>> &vPacket, uint64_t& packetPts, uint64_t& packetDts, int64_t &timeEncodeFinishUs, NV_ENC_PIC_PARAMS *pPicParams)
{
    vPacket.clear();
    ...
    DoEncode(m_vMappedInputBuffers[i], vPacket, packetPts, packetDts, timeEncodeFinishUs, pPicParams);
}
...
void NvEncoder::DoEncode(NV_ENC_INPUT_PTR inputBuffer, std::vector<std::vector<uint8_t>> &vPacket, uint64_t& packetPts, uint64_t& packetDts, int64_t& timeEncodeFinishUs, NV_ENC_PIC_PARAMS *pPicParams)
{
    ...
    {		
        m_vInputFrameInfo.emplace_back(picParams.inputTimeStamp, rtc::TimeMicros());

        m_iToSend++;
        GetEncodedPacket(m_vBitstreamOutputBuffer, vPacket, packetPts, packetDts, timeEncodeFinishUs, true);
    }
    else
    {
...
void NvEncoder::EndEncode(std::vector<std::vector<uint8_t>> &vPacket)
{
	...
	uint64_t unusedPacketPts, unusedPacketDts;
	int64_t unusedTimeEncodeFinishUs;
	
    GetEncodedPacket(m_vBitstreamOutputBuffer, vPacket, unusedPacketPts, unusedPacketDts, unusedTimeEncodeFinishUs, false);
}

void NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, std::vector<std::vector<uint8_t>> &vPacket, uint64_t &packetPts, uint64_t&packetDts, int64_t& timeEncodeFinishUs, bool bOutputDelay)
{
    unsigned i = 0;
    ...
    for (; m_iGot < iEnd; m_iGot++)
    {
        ...
        packetPts = lockBitstreamData.outputTimeStamp;
        auto inputInfo = m_vInputFrameInfo.front();
		packetDts = inputInfo.inputTimeStamp;
		timeEncodeFinishUs = inputInfo.encodeFinishTimeUs;
        m_vInputFrameInfo.erase(m_vInputFrameInfo.begin());
		
        //LOG("GetEncodedPacket. pict.Type(%d), pts(%lld), dts(%lld), bytes(%d), timestamp.q(%d), frameIntervalP(%d)", 
        //    lockBitstreamData.pictureType, 
        //    packetPts,
        //    packetPts,
        //    lockBitstreamData.bitstreamSizeInBytes, 
        //    m_vTimestampList.size(), 
...
void NvEncoder::DoMotionEstimation(NV_ENC_INPUT_PTR inputBuffer, NV_ENC_INPUT_PTR inputBufferForReference, std::vector<uint8_t> &mvData)
{
    ...
    if (nvStatus == NV_ENC_SUCCESS || nvStatus == NV_ENC_ERR_NEED_MORE_INPUT)
    {
        ...
        GetEncodedPacket(m_vMVDataOutputBuffer, vPacket, unusedPacketPts, unusedPacketDts, unusedTime, true);
        if (vPacket.size() != 1)
        {
