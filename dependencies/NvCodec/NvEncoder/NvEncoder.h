...
public:
    ...
    */
    void EncodeFrame(std::vector<std::vector<uint8_t>> &vPacket, uint64_t& packetPts, uint64_t& packetDts, int64_t &timeEncodeFinishUs, NV_ENC_PIC_PARAMS *pPicParams = nullptr);
...
private:
    ...
    */
    void DoEncode(NV_ENC_INPUT_PTR inputBuffer, std::vector<std::vector<uint8_t>> &vPacket, uint64_t& packetPts, uint64_t& packetDts, int64_t& timeEncodeFinishUs, NV_ENC_PIC_PARAMS *pPicParams);
    ...
    */
    void GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, std::vector<std::vector<uint8_t>> &vPacket, uint64_t& packetPts, uint64_t& packetDts, int64_t& timeEncodeFinishUs, bool bOutputDelay);
...
private:
    ...
    struct NvEncInputFrameInfo
    {
		uint64_t inputTimeStamp;
        int64_t encodeFinishTimeUs;
    };
	std::vector<NvEncInputFrameInfo> m_vInputFrameInfo;
};
